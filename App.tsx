...
const MinusIcon: React.FC<{className?: string}> = ({ className }) => ( <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}> <path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14" /> </svg> );
...
const Dropzone: React.FC<DropzoneProps> = ({ onFileSelect, uploadedImage, onClear, helpText, disabled = false, compact = false }) => {
    ...
    const dropzoneClasses = `relative flex items-center justify-center w-full h-full glass-input rounded-3xl overflow-hidden transition-all duration-500 ${
        ...
        >
            {uploadedImage ? (
                ...
                <div className="p-4 flex flex-col items-center">
                    <UploadIcon />
                    {!compact && <p className="text-white/60 text-sm font-medium mt-2">{helpText}</p>}
                </div>
            )}
...
const ComicCanvas: React.FC<ComicCanvasProps> = ({ frames, setFrames, aspectRatio, onCanvasReady }) => {
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        if (selectedIndex !== null && safeFrames[selectedIndex]) {
            ...
            for (const [k, h] of Object.entries(hls)) { if (x >= h.x && x <= h.x+h.w && y >= h.y && y <= h.y+h.h) { setActiveHandle(k); return; } }
        }
        let found = -1;
        ...
        if (found !== -1) { setSelectedIndex(found); } else { setSelectedIndex(null); setStartPos({x, y}); setIsDrawing(true); }
    };
...
const App: React.FC = () => {
    ...
    const handleGenerateClick = useCallback(async () => {
        ...
        try {
            // Build advanced prompt additions
            ...
            const stickerArrays = await generateStickers(uploadedImage.base64, uploadedImage.mimeType, finalRequest, secondaryImage?.base64, secondaryImage?.mimeType);
    ...
    }, [uploadedImage, secondaryImage, prompt, selectedEmotion, selectedBody, selectedStyle, isManipulationEnabled, emotionIntensity]);

    const handleGenerateComic = async () => {
        if (!comicPrompt || comicFrames.length === 0 || comicCharacters.length === 0) { setError('Thiếu thông tin tạo truyện.'); return; }
        setIsLoading(true); setError(null);
        try {
            const layoutDataUrl = comicCanvasRef.current!.toDataURL('image/png');
            const result = await generateComic(comicPrompt, { base64: layoutDataUrl.split(',')[1], mimeType: 'image/png', dataURL: layoutDataUrl }, comicCharacters.filter(c => c.base64), comicRatio);
        ...
        <div className="min-h-screen text-white p-4 sm:p-8">
            ...
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-12">
                    ...
                    <h1 className="text-5xl sm:text-7xl font-bold tracking-tight bg-gradient-to-b from-white to-white/60 text-transparent bg-clip-text mb-4">
                        AI Image Studio
                    </h1>
                ...
                <nav className="flex justify-center mb-12">
                    <div className="glass-card p-1.5 rounded-full flex gap-1">
                        <button onClick={() => setActiveTab('sticker')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all ${activeTab === 'sticker' ? 'bg-white text-black shadow-xl' : 'text-white/50 hover:text-white'}`}>
                            Sticker Studio
                        </button>
                        <button onClick={() => setActiveTab('manga')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all ${activeTab === 'manga' ? 'bg-white text-black shadow-xl' : 'text-white/50 hover:text-white'}`}>
                            Manga Maker
                        </button>
                    </div>
                </nav>
                ...
                <main className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    <section className="lg:col-span-5 glass-card rounded-[40px] p-8 space-y-8">
                        {activeTab === 'sticker' ? (
                            <>
                                <div className="space-y-4">
                                    <h2 className="text-xl font-bold flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-purple-500"></div> 1. Nhập liệu
                                    </h2>
                                    <div className="grid grid-cols-2 gap-4 h-48">
                                        <Dropzone onFileSelect={setUploadedImage} uploadedImage={uploadedImage} onClear={() => setUploadedImage(null)} helpText="Ảnh chính" />
                                        <Dropzone onFileSelect={setSecondaryImage} uploadedImage={secondaryImage} onClear={() => setSecondaryImage(null)} helpText="Ảnh phụ" />
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    <h2 className="text-xl font-bold flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-pink-500"></div> 2. Biểu cảm
                                    </h2>
                                    <div className="grid grid-cols-4 gap-2">
                                        {emotions.slice(0, 12).map(e => (
                                            <button key={e.label} onClick={() => setSelectedEmotion(prev => prev.includes(e.label) ? prev.filter(x => x !== e.label) : [...prev, e.label])} className={`p-3 rounded-2xl text-lg flex items-center justify-center transition-all ${selectedEmotion.includes(e.label) ? 'bg-white text-black scale-95' : 'glass-input hover:bg-white/5'}`}>
                                                {e.emoji}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                {/* RESTORED ADVANCED OPTIONS WITH TOOLTIPS */}
                                <div className="space-y-4">
                                    <button 
                                        onClick={() => setIsAdvancedOpen(!isAdvancedOpen)} 
                                        className="w-full flex justify-between items-center p-4 glass-input rounded-2xl text-white/80 hover:text-white transition-all group"
                                        disabled={!uploadedImage}
                                    >
                                        <span className="font-bold flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-blue-500"></div> Tùy chọn nâng cao
                                        </span>
                                        ...
                                        <div className="space-y-6 p-6 glass-card rounded-3xl animate-in fade-in slide-in-from-top-4 duration-500">
                                            ...
                                            <div>
                                                <div className="flex justify-between mb-3">
                                                    <h4 className="text-xs font-bold text-white/40 uppercase tracking-widest">Cường độ biểu cảm</h4>
                                                    <span className="text-xs font-bold text-blue-400">{emotionIntensity}/10</span>
                                                </div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {bodyOptions.map(option => (
                                                        <div key={option} className="group relative">
                                                            <button 
                                                                onClick={() => setSelectedBody(option)} 
                                                                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${selectedBody === option ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'glass-input text-white/50 hover:text-white'}`}
                                                            >
                                                                {option}
                                                            </button>
                                                            {/* Restored Liquid Glass Tooltip */}
                                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-2 bg-black/60 backdrop-blur-xl border border-white/10 text-[10px] text-white/90 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none w-max max-w-[180px] text-center shadow-2xl z-[70]">
                                                                {bodyOptionDescriptions[option]}
                                                                <div className="absolute top-full left-1/2 -translate-x-1/2 border-8 border-transparent border-t-black/60"></div>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="flex gap-2">
                                                    {styleOptions.map(option => (
                                                        <button key={option} onClick={() => setSelectedStyle(option)} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${selectedStyle === option ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'glass-input text-white/50 hover:text-white'}`}>
                                                            {option}
                                                        </button>
                                                    ))}
                                                </div>
                                ...
                                <div className="space-y-4">
                                    <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Nhập yêu cầu tùy chỉnh tại đây..." className="w-full h-32 glass-input rounded-3xl p-6 text-white placeholder-white/20 outline-none resize-none" />
                                </div>

                                <button onClick={handleGenerateClick} className="w-full py-6 rounded-3xl bg-gradient-to-r from-purple-500 to-pink-500 font-bold text-xl shadow-[0_0_30px_rgba(168,85,247,0.4)] haptic-button">
                                    Bắt đầu tạo ảnh
                                </button>
                            </>
                        ) : (
                            <>
                                <div className="space-y-4">
                                    <h2 className="text-xl font-bold flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-indigo-500"></div> 1. Layout
                                    </h2>
                                    <div className="flex gap-2">
                                        {['1:1', '16:9', '9:16'].map(r => (
                                            <button key={r} onClick={() => { setComicRatio(r as any); setComicFrames([]); }} className={`flex-1 py-3 rounded-2xl text-sm font-bold border ${comicRatio === r ? 'bg-white text-black' : 'border-white/10 hover:bg-white/5'}`}>
                                                {r}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <h2 className="text-xl font-bold flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-pink-500"></div> 2. Nhân vật
                                    </h2>
                                    <div className="grid grid-cols-4 gap-2">
                                        ...
                                        <button onClick={() => setComicCharacters([...comicCharacters, {base64:'',mimeType:'',dataURL:''}])} className="h-16 glass-input rounded-2xl flex items-center justify-center text-white/20 hover:text-white/60 transition-colors">
                                            <PlusIcon className="w-6 h-6" />
                                        </button>
                                    </div>
                                </div>

                                <button onClick={handleGenerateComic} className="w-full py-6 rounded-3xl bg-white text-black font-bold text-xl shadow-2xl haptic-button">
                                    Xuất trang truyện
                                </button>
                            </>
                        )}
                        {error && <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm flex items-center gap-3 animate-pulse"><ExclamationTriangleIcon className="w-5 h-5" /> {error}</div>}
                    </section>

                    <section className="lg:col-span-7 space-y-6">
                        <div className="flex justify-between items-center px-4">
                            <h2 className="text-2xl font-bold tracking-tight">Kết quả sáng tạo</h2>
                            <div className="flex gap-2">
                        ...
                        <div className="glass-card rounded-[40px] p-8 min-h-[600px] flex flex-col">
                            {activeTab === 'sticker' ? (
                                generatedStickers.length === 0 ? (
                                    <div className="flex-grow flex flex-col items-center justify-center text-white/10 space-y-4">
                                        <SparklesIcon className="w-24 h-24" />
                                        <p className="text-lg font-bold tracking-widest uppercase">Trống</p>
                                    </div>
                                ) : (
                                        ...
                                        {generatedStickers.map((s, i) => (
                                            <div key={i} className="group relative glass-card rounded-3xl overflow-hidden aspect-square transition-all hover:scale-[1.02] hover:shadow-2xl">
                                                ...
                                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                                                    <button onClick={() => { const l = document.createElement('a'); l.href = s; l.download = 'ai-art.png'; l.click(); }} className="p-3 bg-white text-black rounded-full hover:scale-110 transition-transform"><DownloadIcon className="w-6 h-6" /></button>
                                                    <button onClick={() => setPreviewedSticker({url:s, index:i})} className="p-3 bg-white text-black rounded-full hover:scale-110 transition-transform"><SparklesIcon className="w-6 h-6" /></button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                !generatedComic ? (
                                    <div className="flex-grow flex flex-col items-center justify-center text-white/10 space-y-4">
                                        ...
                                        <p className="text-lg font-bold tracking-widest uppercase">Chưa có truyện</p>
                                    </div>
                                ) : (
                                    <div className="space-y-6">
                                        <img src={generatedComic} className="w-full rounded-3xl border border-white/10 shadow-2xl" alt="" />
                                        <button onClick={() => { const l = document.createElement('a'); l.href = generatedComic; l.download = 'comic.png'; l.click(); }} className="w-full py-4 glass-input rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-white hover:text-black">
                                            <DownloadIcon className="w-5 h-5" /> Tải xuống trang này
                                        </button>
                                    </div>
                                )
                            )}
            ...
            {previewedSticker && (
                <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-2xl flex items-center justify-center p-8" onClick={() => setPreviewedSticker(null)}>
                    <div className="relative max-w-4xl w-full" onClick={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => setPreviewedSticker(null)} className="absolute -top-12 right-0 text-white/60 hover:text-white">
